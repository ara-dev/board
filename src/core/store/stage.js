...
export default class StageOptionStore {
    ...
    set selectedElements(shapes) {
        ...
        if (shapes.length == 1) {
            ...
            if (selectedShape.getClassName() == 'Text') {
                this.setTextOptions(selectedShape);
    ...
    applyTextAlign() {
        this._state.selectedElements.forEach((item) => {
            ;
            item.align(this._state.textOption.align);
        });
    }
    applyDuplicate() {
        const group = this.getGroup();
    ...
    applyPaste() {
        const shapes = this._state.copyElements.map((item) => {
    ...
    applyDeleteAll() {
        this.getAllShapes().forEach((item) => {
            item.destroy();
        });
    }
    applySelectAll() {
        const shapes = this.getAllShapes().filter((item) => {
    ...
    applyAlignLeft() {
        ...
        const distanceX = this._state.selectedElements.map((item) => {
            return item.getClientRect().x;
        });
        ...
        children.forEach((item) => {
            ...
            if (shape.className == 'Rect') {
                item.clipFunc(function (ctx) {
                    ctx.rect(_.get(shape, 'attrs.x', 0), _.get(shape, 'attrs.y', 0), _.get(shape, 'attrs.width', 0), _.get(shape, 'attrs.height', 0));
                });
            }
    ...
    changeResizeRotateEnableTransformer(isEnable = true, transformer) {
        if (transformer) {
    ...
    setTransformer(stage, layer) {
        ...
        const mouseUp = (e) => {
            ...
            const selected = shapes.filter((shape) => Konva.Util.haveIntersection(box, shape.getClientRect()) && shape.draggable());
    ...
    setSnapping(stage, layer, group) {
        group.on('dragmove', (e) => {
            // clear all previous lines on the screen
            layer.find('.guid-line').forEach((l) => l.destroy());
            // find possible snapping lines
            const lineGuideStops = this.getLineGuideStops(stage, e.target);
